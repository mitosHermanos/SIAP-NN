...
model.add(layers.Embedding(NB_WORDS, 50, input_length=MAX_LEN))
model.add(layers.SpatialDropout1D(0.2))
model.add(layers.LSTM(100, dropout=0.2, recurrent_dropout=0.2))
model.add(layers.Dense(3, activation='softmax'))

model.summary()
# emb_model.save('model')
history = deep_model(model, X_train_emb, y_train_emb, X_valid_emb, y_valid_emb)
results = test_model(model, X_train_seq_trunc, y_train_oh, X_test_seq_trunc, y_test_oh, 10)
print('\n')
print('Test accuracy of word embeddings model: {0:.2f}%'.format(results[1] * 100))
model.save('emb_model')
var = history.history['accuracy'][-1]
print(var)
